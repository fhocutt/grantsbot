...
def main(filepath):
    ...
    try:
        ...
        mblog.logerror(u'Could not construct profile dictionaries', exc_info=True)
        logged_errors = True
        mblog.logrun(filepath, run_time, edited_pages, wrote_db, logged_errors)
        sys.exit()
    ...
    try:
        ...
        mblog.logerror(u'Could not fetch active ideas from idealab_ideas', exc_info=True)
        logged_errors = True
        mblog.logrun()
        sys.exit()
    ...
    for profile in new_profiles:
        ...
        try:
            ...
            mblog.logerror(u'Could not generate ideas for {}'.format(profile['profile_title']), exc_info=True)
            logged_errors = True
            continue

        if len(final_ideas) < 5:
            final_ideas = get_more_ideas(final_ideas, interests, skills, site, config['categories'])
        else:
            pass
        if len(final_ideas) < 5:
            final_ideas = get_more_ideas(final_ideas, [], [], site, config['categories'])
        else:
            pass
...
def get_more_ideas(final_ideas, interests, skills, site, config['categories']):
    """"""
    # this does it twice for skill and interest = [], but that's ok, we filter later
    try:
